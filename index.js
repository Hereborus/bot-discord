...
let tokenRejected=false; // true si Discord a rejeté le token au login
...
function metaPath(userId)   { return path.join(META_DIR, `${userId}.json`); }
function configPath(userId) { return path.join(META_DIR, `${userId}_config.json`); }
function readMeta(userId)   { try { return JSON.parse(fs.readFileSync(metaPath(userId),"utf-8")); } catch { return {}; } }
function writeMeta(userId, meta) { fs.writeFileSync(metaPath(userId), JSON.stringify(meta,null,2)); }
function readUserConfig(userId)  { try { return JSON.parse(fs.readFileSync(configPath(userId),"utf-8")); } catch { return null; } }
function writeUserConfig(userId, cfg) { fs.writeFileSync(configPath(userId), JSON.stringify(cfg,null,2)); }
function stateDir(userId, stateKey) { return path.join(IMAGES_DIR, userId, stateKey); }
...
function getFrames(userId) {
  ...
  for (const state of fs.readdirSync(userDir).filter(s=>fs.statSync(path.join(userDir,s)).isDirectory())) {
    const order=meta[state]||[], dir=stateDir(userId,state);
    const existing=fs.readdirSync(dir).filter(f=>MIME[path.extname(f).toLowerCase()]);
    const ordered=order.filter(f=>existing.includes(f));
    const unordered=existing.filter(f=>!ordered.includes(f));
    result[state]=[...ordered,...unordered].map(f=>({ file:f, url:`/images/${userId}/${state}/${f}` }));
  }
  return result;
}
...
function parseMultipart(body, boundary) {
  ...
  while(offset<body.length) {
    const start=indexOf(body,sep,offset); if(start===-1) break;
    offset=start+sep.length;
    if(body[offset]===45&&body[offset+1]===45) break;
    if(body[offset]===13) offset+=2;
    const headerEnd=indexOf(body,Buffer.from("\r\n\r\n"),offset); if(headerEnd===-1) break;
    const headerStr=body.slice(offset,headerEnd).toString(); offset=headerEnd+4;
    const nextSep=indexOf(body,sep,offset);
    const dataEnd=nextSep===-1?body.length:nextSep-2;
    const data=body.slice(offset,dataEnd); offset=nextSep===-1?body.length:nextSep;
    parts.push({
      name:headerStr.match(/name="([^"]+)"/)?.[1]||"",
      filename:headerStr.match(/filename="([^"]+)"/)?.[1]||"",
      contentType:headerStr.match(/Content-Type:\s*(.+)/i)?.[1]?.trim()||"",
      data,
    });
  }
  return parts;
...
http.createServer(async (req, res) => {
  ...
  if (req.method==="GET" && url.pathname==="/levels") {
    const p={ _bot:{ connected:botConnected, guildId:connectedGuildId, channelId:connectedChannelId, updatedAt:new Date().toISOString() } };
  ...
  if (req.method==="GET" && url.pathname==="/status") {
    return json(res,{ botConnected, connectedGuildId, connectedChannelId, usersActive:userLevels.size });
  }
  ...
  // POST /bot-token — valider + sauvegarder + redémarrer
  if (req.method==="POST" && url.pathname==="/bot-token") {
    try {
      ...
      if (!vRes.ok) { const e=await vRes.json().catch(()=>({})); return json(res, { ok:false, error:`Rejeté par Discord: ${e.message||vRes.status}` }, 401); }
      const botUser = await vRes.json();
      const envPath = path.join(ROOT, ".env");
      let envContent = ""; try { envContent = fs.readFileSync(envPath,"utf-8"); } catch {}
      if (/^DISCORD_TOKEN=.*/m.test(envContent)) envContent = envContent.replace(/^DISCORD_TOKEN=.*/m, `DISCORD_TOKEN=${token.trim()}`);
      else envContent += `
DISCORD_TOKEN=${token.trim()}
`;
      fs.writeFileSync(envPath, envContent, "utf-8");
      console.log(`Token mis a jour pour ${botUser.username} - redemarrage...`);
      json(res, { ok:true, tag:`${botUser.username}#${botUser.discriminator}`, id:botUser.id });
  ...
  if (req.method==="GET" && url.pathname==="/frames") {
    ...
    if (fs.existsSync(IMAGES_DIR)) {
      for (const uid of fs.readdirSync(IMAGES_DIR)) {
        if (fs.statSync(path.join(IMAGES_DIR,uid)).isDirectory()) all[uid]=getFrames(uid);
      }
    }
    return json(res,all);
  }
  ...
  if (req.method==="GET" && url.pathname.startsWith("/images/")) {
    ...
    if (parts.length<4) { res.writeHead(400); res.end(); return; }
    ...
    if (!serveFile(res,fp)) { res.writeHead(404); res.end("Not found"); }
    return;
  }
  ...
  if (req.method==="POST" && url.pathname==="/upload") {
    try {
      ...
      const bm=ct.match(/boundary=(.+)/); if(!bm) return json(res,{error:"boundary manquant"},400);
      const parts=parseMultipart(body,bm[1]);
      const fields={};
      let imgPart=null;
      for (const p of parts) { if(p.filename) imgPart=p; else fields[p.name]=p.data.toString().trim(); }
      const {userId,stateKey}=fields;
      if (!userId||!stateKey||!imgPart) return json(res,{error:"userId, stateKey et image requis"},400);
      ...
      return json(res,{ ok:true, file:fname, url:`/images/${userId}/${stateKey}/${fname}` });
    } catch(err) { return json(res,{error:err.message},500); }
  }

  // GET /user-config/:userId — lire la config audio d'un user
  if (req.method==="GET" && url.pathname.startsWith("/user-config/")) {
    const userId = url.pathname.split("/")[2];
    if (!userId) return json(res,{error:"userId manquant"},400);
    const cfg = readUserConfig(userId);
    return json(res, cfg || {});
  }

  // POST /user-config/:userId — sauvegarder la config audio d'un user
  if (req.method==="POST" && url.pathname.startsWith("/user-config/")) {
    try {
      ...
      console.log(`💾 Config sauvée: ${userId}`);
      return json(res,{ok:true});
    } catch(err) { return json(res,{error:err.message},500); }
  }

  // GET /known-users — liste tous les users ayant des frames ou une config
  if (req.method==="GET" && url.pathname==="/known-users") {
    const users = [];
    // Users avec des frames (dossier images)
    if (fs.existsSync(IMAGES_DIR)) {
      for (const uid of fs.readdirSync(IMAGES_DIR)) {
        if (!fs.statSync(path.join(IMAGES_DIR,uid)).isDirectory()) continue;
        const cfg = readUserConfig(uid);
        users.push({ userId: uid, displayName: cfg?.displayName || uid, hasConfig: !!cfg });
      }
    }
    // Users avec config mais sans dossier images encore
    if (fs.existsSync(META_DIR)) {
      for (const f of fs.readdirSync(META_DIR)) {
        if (!f.endsWith('_config.json')) continue;
        const uid = f.replace('_config.json','');
        if (users.find(u=>u.userId===uid)) continue;
        const cfg = readUserConfig(uid);
        users.push({ userId: uid, displayName: cfg?.displayName || uid, hasConfig: true });
      }
    }
  ...
  if (req.method==="POST" && url.pathname==="/reorder") {
    try {
      const {userId,stateKey,order}=JSON.parse((await readBody(req)).toString());
      if (!userId||!stateKey||!Array.isArray(order)) return json(res,{error:"params manquants"},400);
      const meta=readMeta(userId); meta[stateKey]=order; writeMeta(userId,meta);
      return json(res,{ok:true});
    } catch(err) { return json(res,{error:err.message},500); }
  }
  ...
  if (req.method==="POST" && url.pathname==="/delete-frame") {
    try {
      const {userId,stateKey,file}=JSON.parse((await readBody(req)).toString());
      if (!userId||!stateKey||!file) return json(res,{error:"params manquants"},400);
      const fp=path.join(IMAGES_DIR,userId,stateKey,file);
      if (!fp.startsWith(IMAGES_DIR)) return json(res,{error:"Interdit"},403);
      if (fs.existsSync(fp)) fs.unlinkSync(fp);
      const meta=readMeta(userId);
      if (meta[stateKey]) meta[stateKey]=meta[stateKey].filter(f=>f!==file);
      writeMeta(userId,meta);
  ...
  console.log(`  ├─ Viewer OBS : http://localhost:${PORT}/viewer.html?userId=DISCORD_ID`);
...
function computeFreqBands(buffer) {
  ...
  try {
    ...
    const binHz=AUDIO.sampleRate/AUDIO.fftSize, result={};
    for (const [name,band] of Object.entries(AUDIO.freqBands)) {
      let sum=0,count=0;
      for (let i=Math.floor(band.min/binHz); i<=Math.ceil(band.max/binHz)&&i<mags.length; i++) { sum+=mags[i]; count++; }
...
function subscribeUser(receiver, userId, displayName) {
  try {
    ...
    decoder.on("data",(chunk) => {
      for (let i=0;i<chunk.length;i+=2) { const s=chunk.readInt16LE(i); sumSq+=s*s; sampleCount++; freqBuf.push(s/32768); if(freqBuf.length>AUDIO.fftSize*2) freqBuf.shift(); }
    });
    const cleanup=() => {
      clearInterval(tick);
      try{opusStream.destroy();}catch(_){}; try{decoder.destroy();}catch(_){};
  ...
  } catch(err) { console.error(`Audio error (${userId}):`,err); }
}
...
client.on("messageCreate", async (message) => {
  ...
  if (message.content==="!join") {
    ...
    try {
      ...
      receiver.speaking.on("start",(userId)=>{
        ...
        // Persister le displayName pour /known-users
        const existing = readUserConfig(userId) || {};
        if (existing.displayName !== displayName) {
          writeUserConfig(userId, { ...existing, displayName });
        }
      });
      ...
      const links=members.map(m=>`  • ${m.displayName}: http://localhost:${PORT}/viewer.html?userId=${m.id}`).join("\n");
  ...
  if (message.content==="!status") {
    const lines=[...userLevels.entries()].map(([id,v])=>`  • **${v.displayName||id}** — ${v.db} dB ${v.speaking?"🎙":"🔇"}`).join("\n")||"  (aucun)";
...
if (!process.env.DISCORD_TOKEN) { console.warn("⚠ DISCORD_TOKEN absent — configure-le via http://localhost:" + PORT + "/index.html (onglet Bot Setup)"); } else {
