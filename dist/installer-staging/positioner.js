/**
 * Position Editor Script (`positioner.js`)
 * ========================================
 *
 * Objectif:
 * - Éditer la position/scale des frames d'un état pour un utilisateur (token opaque).
 *
 * Flux:
 * 1. Lit `t` (token) et `state` depuis l'URL.
 * 2. Charge les frames via `GET /frames/:token`.
 * 3. Affiche les layers avec drag + sliders.
 * 4. Persiste `{x,y,s}` en localStorage.
 * 5. Notifie le viewer via `BroadcastChannel('pngtuber-positions')`.
 */
...
function posKey(file) {
    ...
    // Granularité par frame: permet d'avoir un placement indépendant pour chaque image.
...
function saveAll() {
    // Snapshot complet: on persiste tout l'état pour éviter les demi-sauvegardes.
...
async function loadFrames() {
    // Le backend renvoie déjà les URLs relatives des frames classées par état.
    const res = await fetch(`${API_BASE}/frames/${USER_ID}`, {
        cache: "no-store",
    });
...
function renderLayers() {
    ...
    for (const f of stateFrames) {
        ...
        // Drag manuel: on capture la position initiale pour calculer un delta souris.
        let dragging = false,
            startX,
            startY,
            startPosX,
...
function applyTransform(file) {
    ...
    if (!layer) return;
    // Transform CSS non destructive: on ne touche ni à la source ni à la taille native.
    const p = positions[file] || { x: 0, y: 0, s: 1 };
...
function selectFrame(file) {
    ...
    // Z-index
    document
        .querySelectorAll("#layers-container .layer")
        .forEach((l) => (l.style.zIndex = "1"));
    ...
    document.getElementById("selected-frame-name").textContent =
        file.length > 28 ? "…" + file.slice(-25) : file;
...
function onSliderChange(axis, val) {
    ...
    // Les sliders écrivent dans le même store que le drag pour rester cohérent.
    if (!positions[selectedFile])
        positions[selectedFile] = { x: 0, y: 0, s: 1 };
...
// ════════════════════════════════════════════════════════════════
if (!USER_ID) {
    // Guard clause: cette page nécessite un token utilisateur dans l'URL.
